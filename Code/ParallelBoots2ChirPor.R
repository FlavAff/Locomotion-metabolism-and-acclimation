...
chir <- subset(chir, chir$site == "Porto")
...
runstrap <- function(data) {
  fit_boots <- data %>% 
    ...
    group_by(boot_num) %>%
    mutate(fit = map(strap, ~nls_multstart(LogC ~ SchoolfieldMPor(B0, b, m = Mean.mass, E, Ed, TempH, Temp = chamber.T.Kelvin),
...
foreach(i=iter(c(N:1)),
  ...
  write.csv(params_boot,paste0("../Results/SchoolField/Boots/Chironomus/Porto/Boot",i,".csv"))
}
