...
SiteSearchRate.Vopt4.version1 <- function(sit,sppC,sppR,consumer,resource,maxi){
  ...
  p <- ggplot() + theme_classic() +
    ...
    geom_line(data = aS2D, aes(x = Temperature, y = sessile2D, colour = "2D sessile prey"), size = I(1), alpha = 0.4) + 
  ...
  p <- p + scale_color_discrete(name = "Model Strategy") + xlim(5,30) + ylim(0,maxi)#+ ggtitle(paste(sit,"Model Predictions")) + theme(plot.title = element_text(face="bold")) 
  ...
  write.csv(a.results,paste0("../Results/FurtherMods/Corrected2/a_predictions_",sit,resource,"_Vopt4_v1.csv"))
}
