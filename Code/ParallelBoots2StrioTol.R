...
spp <- subset(strio, strio$site == "Toledo")
...
runstrap <- function(data) {
  fit_boots <- data %>% 
    ...
    group_by(boot_num) %>%
    mutate(fit = map(strap, ~nls_multstart(LogC ~ SchoolfieldMTol(B0, b, m = Mean.mass, E, Ed, TempH, Temp = chamber.T.Kelvin),
...
foreach(i=iter(c(N:1)),
  ...
  write.csv(params_boot,paste0("../Results/SchoolField/Boots/Sympetrum/Toledo/Boot",i,".csv"))
}
