...
print("Runninng Schoolfield no mass")

ChirToNoSc <- Fit_NoSchoolfield(tries,ChirToNoR,subset(chir, chir$site == "Toledo"),14.56+273.15)
ChirEvNoSc <- Fit_NoSchoolfield(tries,ChirEvNoR,subset(chir, chir$site == "Evora"),17.30+273.15)
ChirPoNoSc <- Fit_NoSchoolfield(tries,ChirPoNoR,subset(chir, chir$site == "Porto"),16.95+273.15)
...
DiptToNoSc <- Fit_NoSchoolfield(tries,DiptToNoR,subset(dipt, dipt$site == "Toledo"),14.56+273.15)
DiptEvNoSc <- Fit_NoSchoolfield(tries,DiptEvNoR,subset(dipt, dipt$site == "Evora"),17.30+273.15)
DiptPoNoSc <- Fit_NoSchoolfield(tries,DiptPoNoR,subset(dipt, dipt$site == "Porto"),16.95+273.15)

StriToNoSc <- Fit_NoSchoolfield(tries,StriToNoR,subset(strio, strio$site == "Toledo"),14.56+273.15)
StriEvNoSc <- Fit_NoSchoolfield(tries,StriEvNoR,subset(strio, strio$site == "Evora"),17.30+273.15)
StriPoNoSc <- Fit_NoSchoolfield(tries,StriPoNoR,subset(strio, strio$site == "Porto"),16.95+273.15)
...
print("Runninng Schoolfield mass corrected")

ChirToNoSc.corr <- Fit_NoSchoolfield.corr(tries,ChirToNoR,subset(chir, chir$site == "Toledo"),14.56+273.15)
ChirEvNoSc.corr <- Fit_NoSchoolfield.corr(tries,ChirEvNoR,subset(chir, chir$site == "Evora"),17.30+273.15)
ChirPoNoSc.corr <- Fit_NoSchoolfield.corr(tries,ChirPoNoR,subset(chir, chir$site == "Porto"),16.95+273.15)
...
DiptToNoSc.corr <- Fit_NoSchoolfield.corr(tries,DiptToNoR,subset(dipt, dipt$site == "Toledo"),14.56+273.15)
DiptEvNoSc.corr <- Fit_NoSchoolfield.corr(tries,DiptEvNoR,subset(dipt, dipt$site == "Evora"),17.30+273.15)
DiptPoNoSc.corr <- Fit_NoSchoolfield.corr(tries,DiptPoNoR,subset(dipt, dipt$site == "Porto"),16.95+273.15)

StriToNoSc.corr <- Fit_NoSchoolfield.corr(tries,StriToNoR,subset(strio, strio$site == "Toledo"),14.56+273.15)
StriEvNoSc.corr <- Fit_NoSchoolfield.corr(tries,StriEvNoR,subset(strio, strio$site == "Evora"),17.30+273.15)
StriPoNoSc.corr <- Fit_NoSchoolfield.corr(tries,StriPoNoR,subset(strio, strio$site == "Porto"),16.95+273.15)
...
print("Runninng Schoolfield massfree")

ChirToMSc <- Fit_MSchoolfield(tries,ChirToMR,subset(chir, chir$site == "Toledo"),14.56+273.15)
ChirEvMSc <- Fit_MSchoolfield(tries,ChirEvMR,subset(chir, chir$site == "Evora"),17.30+273.15)
ChirPoMSc <- Fit_MSchoolfield(tries,ChirPoMR,subset(chir, chir$site == "Porto"),16.95+273.15)
...
DiptToMSc <- Fit_MSchoolfield(tries,DiptToMR,subset(dipt, dipt$site == "Toledo"),14.56+273.15)
DiptEvMSc <- Fit_MSchoolfield(tries,DiptEvMR,subset(dipt, dipt$site == "Evora"),17.30+273.15)
DiptPoMSc <- Fit_MSchoolfield(tries,DiptPoMR,subset(dipt, dipt$site == "Porto"),16.95+273.15)

StriToMSc <- Fit_MSchoolfield(tries,StriToMR,subset(strio, strio$site == "Toledo"),14.56+273.15)
StriEvMSc <- Fit_MSchoolfield(tries,StriEvMR,subset(strio, strio$site == "Evora"),17.30+273.15)
StriPoMSc <- Fit_MSchoolfield(tries,StriPoMR,subset(strio, strio$site == "Porto"),16.95+273.15)
...
ParasNo <- rbind(ChirToNoParas,ChirEvNoParas,ChirPoNoParas,DiptToNoParas,DiptEvNoParas,DiptPoNoParas,StriToNoParas,StriEvNoParas,StriPoNoParas)
ParasNo.corr <- rbind(ChirToNoParas.corr,ChirEvNoParas.corr,ChirPoNoParas.corr,DiptToNoParas.corr,DiptEvNoParas.corr,DiptPoNoParas.corr,StriToNoParas.corr,StriEvNoParas.corr,StriPoNoParas.corr)
ParasM <- rbind(ChirToMParas,ChirEvMParas,ChirPoMParas,DiptToMParas,DiptEvMParas,DiptPoMParas,StriToMParas,StriEvMParas,StriPoMParas)
...
info <- data.frame(matrix(ncol = 4, nrow = 27))
info <- rename(info, c("X1" = "Genus","X2" = "species","X3" = "Model", "X4" = "site"))

info[c(1:3,10:12,19:21),1] <- "Chironomus"
info[c(4:6,13:15,22:24),1] <- "Cloeon"
info[c(7:9,16:18,25:27),1] <- "Sympetrum"

info[c(4:6,13:15,22:24),2] <- "dipterum"
info[c(7:9,16:18,25:27),2] <- "striolatum"

info[1:9,3] <- "No Mass"
info[10:18,3] <- "0.75 Mass Corrected"
info[19:27,3] <- "Free Mass Scaling"

info[c(1,4,7,10,13,16,19,22,25),4] <- "Toledo"
info[c(2,5,8,11,14,17,20,23,26),4] <- "Evora"
info[c(3,6,9,12,15,18,21,24,27),4] <- "Porto"



SchoolFieldRuns <- cbind(info,Models)
write.csv(SchoolFieldRuns, "../Results/SchoolField/New/SchoolFieldRunsMean.csv")
